...
async function fetchReservedDomains() {
    try {
        if (!reservedDomains) {
            const response = await fetch("https://raw.githubusercontent.com/greyrat-dev/register/main/util/reserved.json");
            ...
            searchDomain.placeholder = "e.g. hisoka";
        }
    } catch (error) {
...
async function check() {
    ...
    isSearching = true;
    try {
        const response = await fetch(`https://raw.githubusercontent.com/greyrat-dev/register/main/domains/${search}.json`);
